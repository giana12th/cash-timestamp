    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python \"C:/Users/<ユーザー名>/.claude/hooks/write_timestamp.py\""
          }
        ]
      }
    ],
...
> **パスの注意:** フックの `command` 内では `~` は展開されません（クォートの内側では bash でも展開されない）。スクリプトの場所は**絶対パス**で書きます。プロジェクトに同梱して使う場合は、Claude Code がプロジェクトルートに展開する `$CLAUDE_PROJECT_DIR` を使い、`python "$CLAUDE_PROJECT_DIR/.claude/hooks/write_timestamp.py"` と書く手もあります。

ここでは **Stop** と **PermissionRequest** の2つのイベントに同じスクリプトを登録しています。

- **Stop** ── Claude が1ターンの応答を終えたタイミング（応答完了）
- **PermissionRequest** ── Claude がツール実行の権限確認でユーザーの入力待ちに入ったタイミング

どちらも **「Claude が待ち始めた」瞬間** です。応答を終えて次の指示を待つときも、権限確認のダイアログを出して承認を待つときも、ユーザーの操作が止まっている時間の起点になります。だから両方をトリガーにしてタイムスタンプを更新します。
...
新しいプロジェクトで Claude Code を使い始めるとき、`.vscode/settings.local.json` に `timestampViewer.filePath` を設定する必要があります。これを毎回手作業でやるのは面倒です。そこで **Claude Code のスキル**（カスタムスラッシュコマンド）として自動化しました。

`.vscode/settings.local.json` は「ローカル専用の設定ファイル」で、フルパスなどマシン固有の値を git に混入させずに置けます（拡張機能側の対応については別記事で書きました）。このファイルと `timestamp.txt` はグローバル gitignoreで除外しているため、プロジェクトごとの `.gitignore` を編集する必要はありません。
...
`-W` オプションで Git Bash 形式（`/d/...`）ではなく Windows 形式（`D:/...`）の絶対パスを得ます。

**ステップ2: `.vscode/settings.local.json` を設定**

ファイルが存在しない場合は `.vscode/` ごと新規作成。存在する場合は **既存のキーを消さずに** `timestampViewer.filePath` だけを追加・上書き。
...
`.gitignore` を触るステップはありません。除外はグローバル gitignore に任せているため、プロジェクト側は何も追記しなくて済みます。

### スキルの設計で気をつけたこと

#### 既存設定を壊さない

`.vscode/settings.local.json` に他の設定が入っていることもあります。ファイルを丸ごと上書きすると既存設定が消えてしまうため、スキルの実装では必ずファイルを Read してからマージ Write する手順を踏んでいます。

#### 絶対パスで設定する

`timestampViewer.filePath` には絶対パスを書きます。VSCode 拡張機能が `fs.watch` でディレクトリを監視する際、相対パスだと VSCode の起動ディレクトリによって解釈が変わる可能性があります。`pwd -W` で取得した絶対パスを直接書くことで、どの環境でも確実に動作します。
...
| コンポーネント                         | 場所                      | 役割                                                  |
| -------------------------------------- | ------------------------- | ----------------------------------------------------- |
| フック設定（Stop / PermissionRequest） | `~/.claude/settings.json` | Claude が待ち始めたときに Python スクリプトを呼び出す |
...
2. `~/.claude/settings.json` に Stop / PermissionRequest フック設定を追加
3. `~/.claude/skills/` に `cash-timestamp-setup` スキルを配置
4. Claude Code を再起動
5. VSCode に cash-timestamp 拡張機能をインストール
...
| 課題                                         | 解決策                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------- |
| Claude が待ち始めたタイミングを検知したい    | Stop / PermissionRequest フックを使う                                     |
...
_この記事は Claude Code (claude-sonnet-4-6) が執筆しました。_
